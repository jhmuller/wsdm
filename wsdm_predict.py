...
def df2csv(ddir, fbase, df, index=False):
...
def main():
    ...
    # add to model
    model = base
    cols = ["msno", "bd",] #"gender", "registered_via", "months_since_reg"]
    model = model.merge(mems[cols], how="left", on="msno")
    del mems

    modeldf = model.compute()
    #modeldf.drop_duplicates(subset=["msno"])
    #del modeldf["msno"]
    df2csv(ddir=mdir, fbase="model_mem", df=modeldf, index=False)
    ...
    #modeldf.drop_duplicates(subset=["msno"])
    #del modeldf["msno"]
    df2csv(ddir=mdir, fbase="model_trans", df=modeldf, index=False)
    ...
    #del modeldf["msno"]
    df2csv(ddir=mdir, fbase="model", df=modeldf, index=False)
